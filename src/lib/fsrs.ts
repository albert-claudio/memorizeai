...
/** Number of lapses to mark a card as leech */
...
export function calculateStabilityIncrease(
  ...
  const { w6, w7, w8, w9, w14, w15 } = weights;
  ...
  if (grade === 1) {
    ...
    // Easy: boost the increase portion
    sInc = 1 + (sInc - 1) * (1 + w15);
  }
...
export function updateDifficulty(
  ...
  // Mean reversion factor (how quickly difficulty returns to average)
  const meanReversion = 0.1;
  
  // Grade adjustment: 0=-2, 1=-1, 2=0, 3=1
  const gradeOffset = grade - 2;
  
  // Calculate new difficulty with mean reversion
  const delta = -w5 * gradeOffset;
  let newDifficulty = currentDifficulty + delta;
  
  // Apply mean reversion toward initial difficulty (5)
  newDifficulty = newDifficulty + meanReversion * (INITIAL_DIFFICULTY - newDifficulty);
...
export function getIntervalPreviews(
  ...
  return {
    ...
    1: formatInterval(processReview(currentState, 1, now, config).intervalDays),
    2: formatInterval(processReview(currentState, 2, now, config).intervalDays),
    3: formatInterval(processReview(currentState, 3, now, config).intervalDays),
  };
}
...
export { DEFAULT_WEIGHTS, DEFAULT_RETENTION } from './fsrs-weights';
export type { FSRSWeights } from './fsrs-weights';
