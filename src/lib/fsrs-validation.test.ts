...
describe('Estabilidade: Crescimento Exponencial', () => {
  ...
  test('intervalos crescem de forma não-linear', () => {
    ...
    // Os saltos devem ser maiores a cada revisão (não-linear)
    const jump1 = intervals[1] - intervals[0]
    const jump2 = intervals[2] - intervals[1]
    const jump3 = intervals[3] - intervals[2]
  ...
  test('Easy (grade 3) produz intervalos maiores que Good (grade 2)', () => {
    ...
    expect(resultEasy.intervalDays).toBeGreaterThanOrEqual(resultGood.intervalDays)
